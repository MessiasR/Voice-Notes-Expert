...
export function NewNoteCard({ onNoteCreated }: NewNoteCard) {
    ...
    function handleSaveNote(event: FormEvent) {
        ...
        if (content == '') {
            return toast.error('Adicione um conteúdo antes de salvar!')
        }
        ...
        <Dialog.Root>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
                    <Dialog.Close 
                    onClick={handleClosedNewNote}
                    className="absolute right-1 top-1 rounded-md bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
                        <X className="size-5" />
